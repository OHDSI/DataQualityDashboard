...
writeJsonResultsToTable <- function(connectionDetails,
  ...
  sink(file = "~/Kheiron/DataQualityDashboard/output/testsink.txt")
  print(checkResults)
  sink(NULL)
  df <- do.call(plyr::rbind.fill, checkResults)
  write.csv(df, "~/Kheiron/DataQualityDashboard/output/testjsondf.csv")
  ...
  for (i in 1:ncol(df)){
    if (grepl("_row", colnames(df)[i], fixed=TRUE)) {
