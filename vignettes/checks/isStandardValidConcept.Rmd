...
- *Numerator*: The number of rows with an `_concept_id` that exists in `CONCEPT.concept_id` but does not equal zero, and has `CONCEPT.standard_concept` != 'S', NULL `CONCEPT.standard_concept`, or non-NULL `CONCEPT.invalid_reason`. 
- *Denominator*: The total number of rows in the table with a non-NULL `_concept_id`.
...
WHERE co.concept_id != 0  
  AND (co.standard_concept != 'S' OR co.invalid_reason IS NOT NULL OR co.standard_concept IS NULL) 
```
...
WHERE c1.concept_id != 0  
  AND (c1.standard_concept != 'S' OR c1.invalid_reason IS NOT NULL OR c1.standard_concept IS NULL) 
