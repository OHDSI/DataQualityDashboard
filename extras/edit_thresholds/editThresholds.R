...
editThresholds <- function(fileThresholds, savingDir, fileField=NULL, fileConcept=NULL, fileTable=NULL){
  ...
  .edit_file <- function(file_, thresholds){
    # 1) Get longer table
  ...
  for(level in unique(df_thresholds$Level)){
    ...
    if(level=="Field"){.edit_file(fileField, thresholds)}
    if(level=="Concept"){.edit_file(fileConcept, thresholds)}
    if(level=="Table"){.edit_file(fileTable, thresholds)}
  }
}
